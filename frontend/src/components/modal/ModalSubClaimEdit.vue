    ...
    <div class="relative w-full h-full max-w-6xl md:h-auto">
      <div class="relative bg-white rounded-md block shadow dark:bg-gray-700 overflow-hidden">
        <div class="flex items-center border-b bg-gray-50 py-3 px-6">
          <h5 class="text-xl font-bold tracking-tight mr-2 text-slate-700 dark:text-white">แก้ไขข้อมูลเคลม : {{ initial.claimModel.code_sub }}</h5>
        ...
        <div class="px-8 pb-6 pt-5">
          <div class="grid lg:grid-cols-12 grid-cols-1 gap-4 mb-2">
            <div class="lg:col-span-2 col-span-5">
              ...
              <input v-model="initial.claimModel.code_sub" type="text" class="border bg-gray-50 border-gray-300 text-slate-700 text-sm rounded-md focus:ring-slate-500 focus:border-slate-500 block w-full p-2 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="SVHXXXX">
            </div>
  ...
  setup(props,  {emit}) {
    ...
    const loadDataDefualt = () => {
      axios.get(apiUrl + "/claim/subclaim/subcode/" + props.codesub).then(response => {
        ...
        initial.claimModel.code_sub = response.data.body[0].code_sub
        // set data
    ...
    const updateClaimAndDownload = () => {
      if(initial.claimModel.time_dry != "" || initial.claimModel.date_dry != ""){
        initial.claimModel.type = "เคลมนัดหมาย"
      }
