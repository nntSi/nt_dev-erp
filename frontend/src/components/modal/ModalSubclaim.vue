<template>
  <div tabindex="-1" class="fixed w-full top-0 left-0 flex justify-center items-center right-0 z-30 p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full bg-opacity-75 bg-black">
    <div class="relative w-full h-full max-w-5xl md:h-auto">
        <div class="bg-white rounded-md shadow dark:bg-gray-700 overflow-hidden">
          <div class="flex items-center border-b bg-gray-50 py-3 px-6">
            <h5 class="text-xl font-bold tracking-tight mr-2 text-slate-700 dark:text-white">{{ svh_code }}</h5>
            <button @click="closeBtn" type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto dark:hover:bg-gray-800 dark:hover:text-white">
          ...
          <div class="px-6 py-6">
            <div class="flex mb-6">
              <div>
                <h5 class="text-xl font-bold tracking-tight mr-2 text-gray-900 dark:text-white">การจัดการเคลมตาม, เคลมนัดหมาย</h5>
                <p class="font-normal text-gray-700 dark:text-gray-400">เพิ่มเคลมนัดหมาย และดูข้อมูล</p>
              </div>
              <div class="ml-auto mt-auto">
                <button @click="controlModalStack('open')" class="py-2 ml-auto px-10 border rounded-md bg-slate-600 text-white text-sm hover:bg-slate-700">+ เพิ่มเคลมตาม</button>
              </div>
            </div>
            <div class="relative overflow-x-auto border rounded-md mb-5">
              <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="text-sm text-gray-700 border-b uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                  <tr>
                ...
                <tbody>
                  <tr v-for="item in initial.dataTable" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                      <a @click="showResultSubClaim(item['code_sub'])" href="#" class="hover:underline font-semibold">{{ item['code_sub'] }}</a>
                    </th>
                    <td class="px-6 py-4">
                      {{ item['employee'] }}
                    </td>
                    ...
                    <td class="px-6 py-4">
                      <a href="#" class="hover:underline">ลบข้อมูล</a>
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div class="flex">
              <button @click="closeBtn" class="py-2 px-5 shadow-sm border rounded-md bg-gray-50 hover:bg-gray-100 text-sm ml-auto">ปิด</button>
            </div>
          </div>
        </div>
    </div>
  ...
  <ModalStacklg v-if="initial.modalStackShow">
    <div>
      <h5 class="text-xl font-bold tracking-tight mr-2 text-gray-900 dark:text-white">เพิ่มเคลมตาม</h5>
      <p class="font-normal text-gray-700 dark:text-gray-400 border-b pb-2 mb-2">เพิ่มข้อมูลเคลมนัดหมาย</p>
      <div class="mb-2">
      ...
      <div class="grid grid-cols-2 gap-3 mb-2">
        ...
        <div>
          ...
          <input v-model="initial.subClaimData.inspector_mobile" type="text" class="border bg-gray-50 border-gray-300 text-gray-900 text-sm rounded-md focus:ring-slate-500 focus:border-slate-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" readonly>
        </div>
      </div>
      <div class="grid lg:grid-cols-2 mb-2 grid-cols-1 gap-4">
        ...
        <div>
          ...
          <input v-model="initial.subClaimData.time" type="text" class="border bg-gray-50 border-gray-300 text-gray-900 text-sm rounded-md focus:ring-slate-500 focus:border-slate-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="00:00">
        </div>
      </div>
      <div class="grid lg:grid-cols-2 grid-cols-1 gap-4 mb-2">
        <div>
          ...
          <input v-model="initial.subClaimData.date_dry" type="text" class="border bg-gray-50 border-gray-300 text-gray-900 text-sm rounded-md focus:ring-slate-500 focus:border-slate-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="วันที่ 10 มกราคม 2565">
        </div>
        <div>
          <label class="block mb-2 text-md font-bold text-gray-900 dark:text-white">เวลา</label>
          <input v-model="initial.subClaimData.time_dry" type="text" class="border bg-gray-50 border-gray-300 text-gray-900 text-sm rounded-md focus:ring-slate-500 focus:border-slate-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="00:00">
        </div>
      </div>
      <div class="mb-2">
        <label class="block mb-2 text-md font-bold text-gray-900 dark:text-white">สถานที่</label>
        <input v-model="initial.subClaimData.location" type="text" class=" mb-2 border bg-gray-50 border-gray-300 text-gray-900 text-sm rounded-md focus:ring-slate-500 focus:border-slate-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="สถานที่นัดหมาย">
        <div class="grid lg:grid-cols-2 grid-cols-1 gap-4">
          <div>
            <label class="block mb-2 text-md font-bold text-slate-700 dark:text-white">จังหวัด</label>
            <select v-model="initial.subClaimData.province" @change="showDistrict" class="bg-gray-50 border border-gray-300 text-gray-900 mb-2 text-sm rounded-md focus:ring-slate-500 focus:border-slate-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-slate-500 dark:focus:border-slate-500">
          ...
          <div>
            ...
            <select v-model="initial.subClaimData.district" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-slate-500 focus:border-slate-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-slate-500 dark:focus:border-slate-500">
      ...
      <div class="flex">
        <button @click="addnewSubClaim" class="px-6 ml-auto mr-3 py-2 border rounded-md bg-slate-600 text-white text-sm hover:bg-slate-700">เพิ่มเคลมตาม</button>
        <button @click="controlModalStack('close')" class="px-6 py-2 border rounded-md bg-gray-50 hover:bg-gray-100">ปิด</button>
      </div>
    </div>
  ...
  <ModalStackXLVue v-if="initial.modalSubResultShow">
    <div>
      <div>
        <h5 class="text-xl font-bold tracking-tight mr-2 text-slate-700 dark:text-white">ข้อมูลรหัส : {{ initial.claimModel.sub_code }}</h5>
        <p class="font-normal text-gray-700 dark:text-gray-400 border-b pb-3 mb-5">เพิ่มข้อมูลเคลมนัดหมาย</p>
      </div>
      <div class="grid grid-cols-2 gap-4 mb-3">
        <div class="border rounded-md overflow-hidden">
          <div class="py-2 px-3 bg-gray-50 border-b">
            <h3 class="text-sm font-semibold text-slate-700">กลุ่ม SURVEYHUB อบ.</h3>
          </div>
      ...
      <div class="flex">
        <button @click="closeReultModal" class="border rounded py-2 px-6 bg-gray-50 hover:bg-gray-100 ml-auto font-semibold text-sm hover:border-gray-300">ปิด</button>
      </div>
    </div>
  </ModalStackXLVue>
...
export default defineComponent({
  ...
  setup(props, {emit}) {
    ...
    onMounted(() => {
      ...
      axios.get(apiUrl + "/location/province").then(response => {initial.province = response.data.body;}).then(() => {
    ...
    const showDistrict = () => {
      axios.get(apiUrl + "/location/district/province/" + initial.subClaimData.province).then(response => {
        initial.district = response.data.body;
      });
    };
    // submit
    const addnewSubClaim = () => {
      if(initial.subClaimData.Inspector == "" || initial.subClaimData.date == "" || initial.subClaimData.date_dry == "" || initial.subClaimData.district == 0 || initial.subClaimData.employee == "" || initial.subClaimData.time_dry == "" || initial.subClaimData.time == "" || initial.subClaimData.location == "" || initial.subClaimData.province == 0){
        ...
        axios.post(apiUrl + '/claim/subcreate', initial.claimModel).then(response => {
    ...
    const loadDataTable = () => {
      axios.get(apiUrl + "/claim/subclaim/svhcode/" + props.svh_code).then(response => {
