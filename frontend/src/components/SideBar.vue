<template>
  <aside class="w-80 h-screen" aria-label="Sidebar">
      <div class="overflow-y-auto py-4 px-8 bg-white rounded-xl border dark:bg-gray-800 shadow-lg h-full">
        ...
        <p class="text-xs text-center mt-1">ระบบจัดการเคลมเวอร์ชั่น 1</p>
        <button type="button" class="flex text-gray-400 mt-3 shadow-sm text-left bg-white border font-medium border-gray-300 focus:outline-none hover:bg-gray-50 focus:ring-4 focus:ring-gray-200 rounded-lg text-sm px-5 py-2 mr-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700 w-full">
        ...
        <ul class="space-y-2">
          <li v-for="item in inintail.menu" v-bind:key="item['main']['id']">
            ...
            <ul :id="item['main']['id']" v-if="checkHaveSub(item['sub'])" class="hidden py-1 space-y-1 bg-gray-50 border">
...
const crrtcolorM = (id:string) => {
  if (id == currentBtn.value){
    return "flex items-center border text-white bg-slate-800 hover:text-white hover:bg-slate-800 focus:ring-1 focus:text-white focus:outline-none focus:ring-gray-300 focus:bg-slate-800 font-medium rounded-lg text-sm px-5 py-2.5 w-full text-left mr-2 dark:border-gray-600 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-800"
  } else {
    return "flex items-center border text-gray-500 bg-slate-50 hover:text-white hover:bg-slate-800 focus:ring-1 focus:text-white focus:outline-none focus:ring-gray-300 focus:bg-slate-800 font-medium rounded-lg text-sm px-5 py-2.5 w-full text-left mr-2 dark:border-gray-600 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-800"
  }
}
